...
export default function OnionScan() {
  ...
  async function startScan() {
    ...
    try {
      ...
      setScanStatus('Scan started. Check status after a while.');
    } catch (error) {
  ...
  async function checkStatus() {
    ...
    try {
      const response = await fetch(`${SERVER_URL}/nmap_status/${taskId}`);
      if (!response.ok) throw new Error(`Error: ${response.status}`);
      ...
      if (data.status === "success" && data.download_url) {
        ...
        setScanStatus('Scan is still in progress. Try again later.');
      } else {
